...
/**
 * @brief 将当前帧编码为 JPEG，并以二进制形式返回
 *
 * @param quality JPEG 编码质量（1–100）
 * @return std::vector<uint8_t> 完整的 JPEG 二进制数据
 */
std::vector<uint8_t> Esp32Camera::GetJpegBuffer(int quality)
{
    std::vector<uint8_t> buffer;
    // 用 frame2jpg_cb 把每个 chunk 收集到 buffer 里
    frame2jpg_cb(fb_, quality, [](void *arg, size_t index, const void *data, size_t len) -> unsigned int
                 {
                     auto buf = static_cast<std::vector<uint8_t> *>(arg);
...
/**
 * @brief 将当前帧编码为 JPEG，并以 Base64 文本返回
 *
 * @param quality JPEG 编码质量（1–100）
 * @return std::string Base64 编码的 JPEG 数据
 */
std::string Esp32Camera::GetJpegBase64(int quality)
{
    // 1) 先拿到二进制 JPEG
    std::vector<uint8_t> buf = GetJpegBuffer(quality);

    // 2) 预估 Base64 输出长度：4 * ceil(n/3)
    size_t b64_len = 4 * ((buf.size() + 2) / 3);

    // 分配足够空间（加1留给 '\0'）
    std::vector<uint8_t> out(b64_len + 1);
    size_t olen = 0;
